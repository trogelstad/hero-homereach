...
- End with one warm, high-energy, specific invitation to book a free Hero HomeReach consultation, making it feel like the obvious next move, not a suggestion.
...
- Never use an em dash (the long " - " character) anywhere in your response, for any reason. Use a period, comma, or the word "and" instead. This rule has no exceptions.`;
...
Write with real energy and specificity  -  this should feel like a genuinely exciting personal moment, not a summary of a form. No markdown, no bullets, no headers. End with a natural, energetic invitation to book a free Hero HomeReach consultation.`;
  ...
  try {
    ...
    // Belt-and-suspenders: the prompt bans em dashes, but model output can't
    // be guaranteed word-for-word, so strip any that slip through before
    // this ever reaches the page.
    result = result
      .replace(/\s+—\s+/g, ', ')
      .replace(/—/g, '. ')
      .replace(/\s+,/g, ',')
